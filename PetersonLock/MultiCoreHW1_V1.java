...
public class MultiCoreHW1_V1 {
	...
	public MultiCoreHW1_V1(int No_of_threads,int No_of_cores, int LBound, int UBound,int Initial_mean,int Step_up_for_mean_time, double lambda,int No_of_hits_to_CS,int No_of_iterations){
	...
	public void test() throws InterruptedException{
		...
		for(int j = 0;j<No_of_iterations ;j++){
			...
			for(int p=0;p<No_of_hits_to_CS;p++){
				Algo obj = new Algo(No_of_threads);
		...
		if(args.length == 0){
			...
			if(pow_of_two(No_of_threads)){
				for(int i =LBound ; i<=UBound;i=i+Step_up_for_mean_time ){
					...
					new MultiCoreHW1_V1(No_of_threads,No_of_cores, LBound, UBound,current_mean,Step_up_for_mean_time, lambda,No_of_hits_to_CS, No_of_iterations).test();
				}
