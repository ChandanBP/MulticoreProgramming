...
public class MCP_TAS_EB {
	...
	public MCP_TAS_EB(int No_of_threads,int No_of_cores, int LBound, int UBound,int Initial_mean,int Step_up_for_mean_time, double lambda,int No_of_hits_to_CS,int No_of_iterations){
	...
	public void test(int min,int max) throws InterruptedException{
		
		for(int t=301;t<=350;t++){
			long now = 0;
			long later = 0;
			long cummulative_time = 0;
			System.out.println("min="+min+" max="+t);
			for(int j = 0;j<No_of_iterations ;j++){
				int final_count = 0;
				now = System.currentTimeMillis() ;
				for(int p=0;p<No_of_hits_to_CS;p++){
					TASLock obj = new TASLock(min,t);
		...
		if(args.length == 0){
			...
			try {
	            ...
	            while((line = bufferedReader.readLine()) != null) {
	            	...
	            	case "No_of_iterations" : No_of_iterations = Integer.parseInt(line.toString().split(" = ")[1]);
											break;
	            	case "MIN_DELAY" : min = Integer.parseInt(line.toString().split(" = ")[1]);
	            							break;
	            	case "MAX_DELAY" : max = Integer.parseInt(line.toString().split(" = ")[1]);
	            							break;
			...
			if(pow_of_two(No_of_threads)){
				...
				for(int i =LBound ; i<=UBound;i=i+Step_up_for_mean_time ){
					...
					new MCP_TAS_EB(No_of_threads,No_of_cores, LBound, UBound,current_mean,Step_up_for_mean_time, lambda,No_of_hits_to_CS, No_of_iterations).test(min,max);
				}
